...
def find_archive_annotations(archive_dir, class_names=['qr', 'signature', 'stamp']):
    """
    Find all images in archive directory that have corresponding .txt annotation files.
    Converts YOLO format annotations back to our format.
...
def process_annotation(ann, class_to_id, images_dir, output_dir, split):
    ...
    # Source image path - use image_path if available (for archive images)
...
def visualize_samples(annotations, class_names, images_dir, output_dir, num_samples=20):
    ...
    for ann in samples:
        ...
        # For archive images, use just the filename for label lookup
        if ann.get('is_archive', False):
        ...
        if not label_path:
            print(f"Warning: Label file not found for {lookup_name}")
            continue
        
        # Use source path for archive images, otherwise use images_dir
        if ann.get('is_archive', False) and 'image_path' in ann:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Build YOLOv11 detection dataset')
    parser.add_argument(
        '--include-archive',
        action='store_true',
    ...
    print(f"      JSON annotations: {len(annotations)}")
    ...
    if args.include_archive:
        ...
        print(f"      Archive annotations: {len(archive_annotations)}")
